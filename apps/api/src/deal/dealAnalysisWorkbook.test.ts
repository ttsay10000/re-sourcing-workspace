...
function sampleContext(): UnderwritingContext {
  return {
    ...
    analystContext: {
      ...
      sourceNotes: ["Market/neighborhood context uses internal property details, broker comp packages, and approved comp items only."],
    },
  };
}
...
describe("buildDealAnalysisWorkbook", () => {
  it("creates a styled workbook with visible analysis tabs and auditable model tabs", async () => {
    ...
    const analystNotes = workbook.getWorksheet("Analyst Notes");
    ...
    expect(analystNotes).toBeDefined();
    ...
    const freshnessRow = findRowByLabel(modelGuide, "Manual-change freshness");
    expect(freshnessRow).not.toBeNull();
    expect(modelGuide?.getCell(`A${(freshnessRow ?? 0) + 1}`).value).toContain("saves the current manual underwriting draft");

    expect(assumptions?.getCell("A1").value).toBe("Deal Analysis Workbook");
    expect(analystNotes?.getCell("A1").value).toBe("Internal Analyst Notes");
    expect(analystNotes?.getCell("A5").value).toBe("Listing Summary");
    expect(analystNotes?.getCell("B8").value).toContain("median sale pricing");
