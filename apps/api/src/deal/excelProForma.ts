...
export function buildExcelProForma(ctx: UnderwritingContext): Buffer {
  ...
  const summarySheet = widthSheet(
    sheet([
      ...
      [s("Financing fees", labelStyle), f("Financing!B3", CURRENCY_FMT, formulaValueStyle), "", s("Stabilized cap (MTR yield)", labelStyle), f(`IF(Assumptions!B${assumptionRows.purchasePrice}=0,"",E6/Assumptions!B${assumptionRows.purchasePrice})`, PERCENT_FMT, formulaValueStyle)],
      ...
      [s("Cash required", sectionStyle), f(`ABS('Cash Flow'!C${leveredCfRow})`, CURRENCY_FMT, formulaValueStyle), "", s("MTR vs LTR spread", labelStyle), f(`IF(Assumptions!B${assumptionRows.purchasePrice}=0,"",E12-B6)`, PERCENT_FMT, formulaValueStyle)],
    ]),
    [24, 20, 4, 24, 20]
  );
  summarySheet["!merges"] = [
