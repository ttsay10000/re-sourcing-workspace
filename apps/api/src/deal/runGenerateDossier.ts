...
export async function runGenerateDossier(
  ...
  try {
    ...
    const ctx: UnderwritingContext = {
      ...
      unitModelRows: detailedModel.unitModelRows.map((row) => ({
        rowId: row.rowId,
        ...
        isVacantLike: row.isVacantLike,
        modeledAnnualRent: row.modeledAnnualRent,
        defaultProjectedAnnualRent: row.defaultProjectedAnnualRent,
        beds: row.beds,
        baths: row.baths,
        sqft: row.sqft,
