  ...
  try {
      ...
      `SELECT
       ...
       LEFT JOIN LATERAL (
         ...
         FROM (
           ...
           FROM deal_signals h
           WHERE h.property_id = p.id AND h.asset_cap_rate IS NOT NULL
         ) t
       ) hist ON TRUE
    ...
    let rows: OperatingCompRow[] = result.rows
      .map((row) => {
        ...
        const rawHistory = Array.isArray(row.yield_history) ? (row.yield_history as Array<Record<string, unknown>>) : [];
