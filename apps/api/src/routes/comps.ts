...
/** Fields the board-status derivation needs, degrading before migrations 053/066. */
...
router.get("/comps/operating", async (req: Request, res: Response) => {
  ...
  try {
    ...
    // migrations (053 rejections, 066 exclusion flag) are still pending.
      ...
      const pendingResult = await pool.query(
        `SELECT
         ...
         LEFT JOIN LATERAL (
           ...
           ORDER BY w.created_at DESC
           LIMIT 1
         ) wsp ON TRUE${rejectionJoin(hasRejections)}
