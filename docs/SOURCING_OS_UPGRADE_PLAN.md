...
| # | Finding | Where | Severity |
|---|---|---|---|
| M1 | **Resolved at implementation — CoC is consistent.** Deep re-verification showed the app's CoC series is `(NOI − debt service) / equity` (`underwritingModel.ts:1358-1360`), matching the dossier Excel (`excelProForma.ts:549`; debt service stored negative at `:513`) and the deal-analysis workbook (`dealAnalysisWorkbook.ts:1793`). Recurring capex is intentionally excluded from CoC in all three. Both earlier automated findings (sign error; capex divergence) were false positives. Remaining action: document the CoC definition in FINANCIAL_FLOWS.md. | `excelProForma.ts:549` | Resolved |
